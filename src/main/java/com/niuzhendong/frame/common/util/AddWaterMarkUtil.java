...
public class AddWaterMarkUtil {
     ...
     * @return 是否成功添加
     */
    public boolean addWaterMark(String wordPath, String waterMarkPath, int left, int top, int width, int height) {
        try {
            if (initWordObj()) {
                openDocument(wordPath);
                getActiveWindow();
                setWaterMark(waterMarkPath,left,top,width,height);
                closeDocument();
                closeWordObj();
                return true;
     ...
     *            水印路径
     */
    public void setWaterMark(String waterMarkPath, int left, int top, int width, int height) {
        // 取得活动窗格对象
    ...
    public static void main(String[] args) {
		...
		boolean b = awmu.addWaterMark("D:/test/new3.docx","D:/test/test3.png",-10,100,450,450);
		if(b){
